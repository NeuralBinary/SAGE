...
SAGE configuration uses environment variables with the `SAGE_` prefix (an optional `.env` file is also read). Names below use their full environment form. Development defaults favor local operation; `SAGE_ENV=production` enables strict fail-closed validation.

The tables below are extracted from `docs/CONFIGURATION.md` and verified against the defaults in `src/sage_plugin/config.py`.

## Runtime and database

| Variable | Default | Purpose |
| --- | --- | --- |
| `SAGE_ENV` | `development` | Runtime environment (`development`, `test`, or `production`). `production` enables strict startup validation. |
...
| `SAGE_API_KEYS` | empty | Comma-separated (or JSON-array) service keys. Each configured key must contain at least 32 characters when auth is enabled. |
| `SAGE_AGENT_KEYS` | empty | JSON mapping from secret key to `agent` or `workspace:agent` scope. |
| `SAGE_ALLOWED_HOSTS` | empty | Comma-separated (or JSON-array) Host allowlist. Production requires explicit entries. |
...
| `SAGE_SEMANTIC_LSH_BITS` | `10` | Bit width used by deterministic locality-sensitive-hash buckets for large vocabularies (must be in [4, 24]). |
| `SAGE_SEMANTIC_LSH_HAMMING` | `1` | Neighbor Hamming radius used for bounded large-vocabulary lookup (must be in [0, 2]). |
...
| Variable | Default | Purpose |
| --- | --- | --- |
| `SAGE_EMBEDDING_PROVIDER` | `hash` | Embedding backend (`hash`, `openai`, or `openai_compatible`). Deterministic hash mode is local/offline. |
...
Production mode (`SAGE_ENV=production`) requires **all** of the following, or the service refuses to start:

1. `SAGE_AUTH_REQUIRED=true` — authentication is mandatory.
2. One or more service API keys of at least 32 characters (each key in `SAGE_API_KEYS`), and agent keys (`SAGE_AGENT_KEYS`) also at least 32 characters. Service and agent keys must be disjoint sets.
3. A **server database URL** — SQLite is rejected (`SAGE_DATABASE_URL` must not start with `sqlite`). Use PostgreSQL (e.g. `postgresql+psycopg://...`).
4. `SAGE_AUTO_CREATE_SCHEMA=false` — schema is managed by Alembic migrations (`alembic upgrade head`) as a one-shot deployment step.
5. Explicit `SAGE_ALLOWED_HOSTS` — wildcard-only configuration is rejected.
6. `SAGE_DOCS_ENABLED=false` — interactive docs/OpenAPI are disabled in production.
7. External learned embedding endpoints (`SAGE_EMBEDDING_PROVIDER=openai` or `openai_compatible`) must use HTTPS URLs.

Additional validation that applies in any environment:

- `SAGE_REQUIRE_PACKET_SIGNATURES=true` requires a configured `SAGE_PACKET_SIGNING_PUBLIC_KEY`; signing keys must be URL-safe base64 decoding to exactly 32 bytes.
- `SAGE_REQUIRE_REF_ENCRYPTION=true` requires `SAGE_REF_ENCRYPTION_KEY` (URL-safe base64, exactly 32 bytes).
- Ratio settings must be in `[0, 1]`; budgets/leases/retention must be positive; `SAGE_SEMANTIC_LSH_BITS` in `[4, 24]`; `SAGE_SEMANTIC_LSH_HAMMING` in `[0, 2]`; `SAGE_BACKPRESSURE_DEGRADED_RATIO` must be less than `SAGE_BACKPRESSURE_THROTTLED_RATIO`; trust-scope source thresholds must be positive and nondecreasing.
...
A local environment template ships as `.env.example` (adapter settings plus a commented production Compose example). See also [Production](Production.md) and the repo's `docs/SECURITY.md` and `docs/THREAT_MODEL.md`.
