...
def config_for(platform: str, base_url: str, agent_id: str, workspace: str = "default") -> IntegrationConfigResponse:
    ...
    if p.id == "hermes":
        ...
        commands = ['unzip sage-hermes-plugin-v0.2.3.zip -d "${HERMES_HOME:-$HOME/.hermes}/plugins"', "hermes plugins enable sage"]
    elif p.id == "openclaw":
        config["remote_mcp_url"] = mcp_url
        files["env"] = f"SAGE_URL={base}\nSAGE_AGENT_ID={agent_id}\nSAGE_WORKSPACE={workspace}\n"
        commands = ["openclaw plugins install npm-pack:./sage-agent-openclaw-sage-0.2.3.tgz", "openclaw plugins enable sage", "openclaw plugins inspect sage --runtime --json"]
    elif p.id == "claude":
        config["remote_mcp_url"] = mcp_url
        ...
        commands = ["pip install ./sage_agent_protocol-0.2.3-py3-none-any.whl", "use SageRuntime or POST /v1/bus/handoff"]
