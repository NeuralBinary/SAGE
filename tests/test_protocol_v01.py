...
def test_canonical_encoding_is_order_independent_and_stable():
    a = {"v": 1, "c": "global", "a": "report", "p": {}, "m": {"z": 1, "a": 2}}
    b = {"m": {"a": 2, "z": 1}, "p": {}, "a": "report", "c": "global", "v": 1}
    assert canonical_json_bytes(a) == canonical_json_bytes(b)
    assert canonical_msgpack_bytes(a) == canonical_msgpack_bytes(b)
    assert canonical_digest(a) == canonical_digest(b)


def test_wire_v1_rejects_unknown_fields_and_nonfinite_values():
    with pytest.raises(Exception):
        validate_wire_v1({"v": 1, "c": "global", "a": "report", "p": {}, "wat": 1})
...
def test_a2a_v1_message_and_agent_card_binding():
    ...
    assert card["capabilities"]["extensions"][0]["params"]["wireVersion"] == 1
...
def test_protocol_api_reports_frozen_v01_and_validates_vectors():
    with TestClient(app) as client:
        info = client.get("/v1/protocol")
        assert info.status_code == 200
        assert info.json()["protocol"] == "sage/0.1"
        assert info.json()["wire_version"] == 1
        tck = client.get("/v1/protocol/tck")
        assert tck.status_code == 200
        assert tck.json()["ok"] is True
        wire = {"v": 1, "c": "global", "a": "report", "p": {}}
        ...
        assert schema.json()["properties"]["v"]["default"] == 1
...
def test_packaged_wire_schema_matches_reference_model():
    ...
    packaged = json.loads(files("sage_plugin").joinpath("spec/wire-v1.schema.json").read_text(encoding="utf-8"))
    assert packaged == wire_schema()


def test_v01_is_the_only_supported_protocol_and_wire():
    from sage_plugin.protocol_spec import SAGE_SUPPORTED_PROTOCOLS, SAGE_SUPPORTED_WIRES

    assert SAGE_SUPPORTED_PROTOCOLS == ("sage/0.1",)
    assert SAGE_SUPPORTED_WIRES == (1,)
    with pytest.raises(Exception):
        validate_wire_v1({"v": 99, "c": "global", "a": "report", "p": {}})
...
def test_packaged_protocol_spec_and_protobuf_binding_present():
    ...
    spec = root.joinpath("SAGE-0.1.md").read_text(encoding="utf-8")
    proto = root.joinpath("sage-v0.1.proto").read_text(encoding="utf-8")
    assert "initial protocol baseline" in spec
    assert 'package sage.v01;' in proto
    assert 'uint32 wire_version = 1' in proto
    assert 'string protocol = 1' in proto
    pattern_schema = json.loads(root.joinpath('pattern-v0.1.schema.json').read_text(encoding='utf-8'))
