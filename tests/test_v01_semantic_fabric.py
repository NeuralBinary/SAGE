...
def test_recursive_pattern_graph_and_namespace_promotion():
    with SessionLocal() as db:
        ...
        left = store.observe(settings.codebook, left_comp)
        right = store.observe(settings.codebook, right_comp)
        assert left is not None and right is not None
        store.set_status(left.pattern_id, "active")
        store.set_status(right.pattern_id, "active")
        recursive = store._observe_recursive(settings.codebook, units)
